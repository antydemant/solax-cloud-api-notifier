...
def send_telegram_to_all(text: str):
    ...
    for chat_id in subs:
        ...
        try:
            ...
            print(_("❌ Failed to send message to {chat_id}: {e}"))
        ...
        if upload_time_str:
            ...
            if now() - upload_time > timedelta(minutes=5):
                if prev_inverter_online:
                    msg = _("🚨 Inverter has not updated data >5 minutes ({time})").format(time=now_str())
            ...
            if prev_grid_status and current_grid_status != prev_grid_status:
                if current_grid_status == "OFF":
                    ...
                    if prev_state.get("last_off_time"):
                        ...
                        delta = now() - off_time
                        duration_str = _(" (out for {duration})").format(duration=format_duration(delta))
                        prev_state["last_off_time"] = None

                    msg = (
                        _("✅ Power is back! ⏰ {time}{duration}\nSOC: {soc}% | Home: {acpower}W | Grid: {feedin}W")
                        .format(time=now_str(), duration=duration_str, soc=soc, acpower=acpower, feedin=feedin)
                    )

                send_telegram_to_all(msg)
                add_history_entry(prev_state, f"grid_{current_grid_status.lower()}", msg)

            prev_state["grid_status"] = current_grid_status

    else:
        if prev_inverter_online:
            msg = _("🚨 Inverter is unavailable (API not responding) ⏰ {time}").format(time=now_str())
            send_telegram_to_all(msg)
            add_history_entry(prev_state, "inverter_offline", msg)
        prev_state["inverter_online"] = False
